...
export const SingleItem = () => {
  ...
  useEffect(() => {
    let mytodo = todos.find((item) => item.id === Number(id));
    setSingleTodo(mytodo);
  }, [todos, id]);
  return (
    <div>
      <h1>
        {singleTodo?.task} - {singleTodo?.taskStatus ? "Done" : "Not done"}
      </h1>
      <button
        onClick={() => handleToggle(singleTodo.id, !singleTodo.taskStatus)}
      >
        Toggle Todo
      </button>
      <button onClick={() => handleRemove(singleTodo.id)}>Remove</button>
      <Link to={`/todos/${singleTodo.id}/edit`}>
